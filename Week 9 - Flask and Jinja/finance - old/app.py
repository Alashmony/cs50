...
def index():
    ...
    stocks = db.execute("SELECT stock, ROUND(SUM(amount), 2) amount, round(sum(value), 2) buy_price from transactions where user = ? group by stock;", userid)
    i = 0
    for stock in stocks:
        symbol = stock["stock"]
        stock_price = lookup(symbol)
        stocks[i]["price"] = stock_price["price"]
        stocks[i]["total"] = round(stock_price["price"] * stock["amount"], 2)
    ...
    return render_template("index.html", username = username, cash=cash, stocks=stocks, notification=notification)
...
def buy():
    ...
    if request.method == "POST":
        ...
        stocks_val = int(amount) * float(stock_price)
...
def history():
    ...
    cash = round(user_data[0]["cash"], 2)
    transactions = db.execute("SELECT (CASE WHEN buy = true then 'Buy' ELSE 'SELL' END) as type , stock, abs(price) price, abs(amount) amount, timestamp from transactions WHERE user = ?;", user)
    return render_template("history.html", username = username, cash = cash, transactions = transactions)
...
def sell():
    ...
    if request.method =="GET":
        stocks = db.execute("SELECT stock, sum(amount) amount FROM transactions WHERE user = ? GROUP BY stock HAVING sum(amount) > 0;", user)
    ...
    amount_in_hand = db.execute("SELECT sum(amount) amount FROM transactions WHERE stock = ? and user = ?", symbol, user)[0]["amount"]
    ...
    db.execute("INSERT INTO transactions (user, stock, amount, price, value, buy, timestamp) values(?,?,?,?,?,?,?)", user, symbol, amount*-1, float(stock_price)*-1,value*-1, False,timestamp)
